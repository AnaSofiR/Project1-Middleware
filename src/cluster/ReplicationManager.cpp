...
void ReplicationManager::replicateTopic(const std::string& topic, const std::string& message) {
    for (const auto& peer : peerAddresses) {
        sendToPeer(peer, topic, message, true);
    }
}

void ReplicationManager::replicateQueue(const std::string& queue, const std::string& message) {
    for (const auto& peer : peerAddresses) {
        sendToPeer(peer, queue, message, false);
    }
}

void ReplicationManager::sendToPeer(const std::string& address, const std::string& name, const std::string& message, bool isTopic) {
    auto channel = grpc::CreateChannel(address, grpc::InsecureChannelCredentials());
    std::unique_ptr<replication::ReplicationService::Stub> stub = replication::ReplicationService::NewStub(channel);
    ...
    if (isTopic) {
        ...
        if (status.ok()) {
            std::cout << "[Replication] Tópico '" << name << "' replicado a " << address << "\n";
        } else {
