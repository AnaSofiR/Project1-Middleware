...
FailoverManager::FailoverManager(const std::string &self,
...
void FailoverManager::stopMonitoring() { running = false; }
    ...
    for (const auto &addr : peerAddresses) {
      ...
      if (!alive) {
        ...
        if (peerStatus[addr] && failureCounts[addr] >= retryThreshold) {
          ...
          std::cerr << "[Failover] Nodo marcado como caído: " << addr
                    << std::endl;
        }
      } else {
        if (!peerStatus[addr]) {
...
bool FailoverManager::pingPeer(const std::string &address) {
  ...
  if (connected) {
    ...
    std::cerr << "[Failover] ❌ No se pudo conectar con " << address
              << std::endl;
  }
...
void FailoverManager::syncRecoveredPeer(const std::string &peer) {
  ...
  if (status.ok()) {
    if (response.topicmessages_size() == 0 &&
        response.queuemessages_size() == 0) {
      std::cout << "[SYNC] No hay datos que sincornizar desde" << peer
                << std::endl;
    } else {
      if (topicManager_)
        topicManager_->applyState(response);
      if (queueManager_)
        queueManager_->applyState(response);
