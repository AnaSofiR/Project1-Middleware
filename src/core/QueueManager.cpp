...
bool QueueManager::createQueue(const std::string& queue, const std::string& user) {
...
bool QueueManager::deleteQueue(const std::string& queue, const std::string& user) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    if (queueOwners[queue] != user) {
        std::cerr << "[QueueManager] Usuario '" << user << "' no tiene permiso para eliminar la cola '" << queue << "'.\n";
        return false;
    }
...
bool QueueManager::enqueueMessage(const std::string& queue, const std::string& message) {
    std::lock_guard<std::mutex> lock(mutex_);

    if (!queueMessages.count(queue)) {
        std::cerr << "[QueueManager] No se puede enviar mensaje. Cola '" << queue << "' no existe.\n";
        return false;
    }
